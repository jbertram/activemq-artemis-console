...
/**
 * A {@link DataUpdater} for Artemis which immediately adds RBAC information, so Hawtio doesn't have to prepare huge and
 * time consuming query (see {@code processRBAC()} in Hawtio's {@code plugins/rbac/tree-processor.ts}).
 */
public class ArtemisDataUpdater extends DataUpdater {
   ...
   private String transformName(String name, Object args) {
      ...
      return String.format("%s(%s)", name, argArray.stream().map(a -> (String) ((JSONObject) a).get("type")).collect(
         Collectors.joining(",")));
   }
